...
class UsageStatsPlugin(ArtefactPlugin):
    """
    """

    def __init__(self):
        super().__init__()
        self.author = '@AlexisBrignoni & @SwiftForensics'
        self.author_email = ''
                ...
                if len(parts) > 2 and parts[-2] == 'usagestats' and parts[-3] == 'system':
                    ...
                    try:
                        ...
                        if file_found.find('{0}mirror{0}'.format(slash)) >= 0:
                            continue
                        self.process_usagestats(file_found, uid, self.report_folder, 1)
                    except ValueError:
                    ...
                    try:
                        ...
                        if file_found.find('{0}mirror{0}'.format(slash)) >= 0:
                            continue
                        self.process_usagestats(file_found, uid, self.report_folder, 2)
                    except ValueError:
                        pass # uid was not a number

    def get_string_by_token(self, packages, token1, token2 = 0):
    ...
    def ReadUsageStatsV2PbFile(self, input_path):
    ...
    def AddV2EntriesToDb(self, sourced, file_name_int, stats_ob, db, packages):
        cursor = db.cursor()
        # packages
        for usagestat_ob in stats_ob.packages:
            ...
            pkg = self.get_string_by_token(packages, usagestat_ob.package_token)
            alc = ''
        ...
        for event in stats_ob.event_log:
            ...
            if event.HasField('package_token'):
                pkg = self.get_string_by_token(packages, event.package_token)
            if event.HasField('class_token'):
                classy = self.get_string_by_token(packages, event.package_token, event.class_token)
    ...
    def ReadUsageStatsPbFile(self, input_path):
    ...
    def AddEntriesToDb(self, sourced, file_name_int, stats, db):
        cursor = db.cursor()
        # packages
    ...
    def add_xml_or_v1_usagestats_to_db(self, folder, db):
            ...
            if os.path.isfile(filename): # filter dirs
                ...
                if file_name == 'version':
                    ...
                    try:
                        ...
                        try:
                            stats = self.ReadUsageStatsPbFile(filename)
                            err = 0
                        except:
                        ...
                        if stats:
                            if ferr == 1:
                              ...
                              #print('')
                              self.AddEntriesToDb(sourced, file_name_int, stats, db)
                              continue
    ...
    def add_v2_usagestats_to_db(self, folder, db):
        ...
        for filepath in glob.iglob(os.path.join(folder, '**'), recursive=True):
            if os.path.isfile(filepath): # filter dirs
                ...
                try:
                    stats_ob = self.ReadUsageStatsV2PbFile(filepath)
                    self.AddV2EntriesToDb(sourced, file_name_int, stats_ob, db, packages)
                except:
                    logfunc(f'Parse error - Error parsing Protobuf file: {filepath}')

    def process_usagestats(self, folder, uid, report_folder, version):
        ...
        if version == 1:
            self.add_xml_or_v1_usagestats_to_db(folder, db)
        else:
            self.add_v2_usagestats_to_db(folder, db)
