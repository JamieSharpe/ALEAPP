...
class GBoardKeyboardPlugin(ArtefactPlugin):
    """
    """
    ...
    def _processor(self) -> bool:
        ...
        for file_found in self.files_found:
            ...
            if file_found.endswith('trainingcachev2.db'):
                self.read_trainingcachev2(file_found)
            elif file_found.endswith('trainingcache2.db') or file_found.endswith('trainingcache3.db'):
                self.read_trainingcache2(file_found)

        return True

    def read_trainingcache2(self, file_found):
    ...
    def read_trainingcachev2(self, file_found):
        db = open_sqlite_db_readonly(file_found)
        db.row_factory = sqlite3.Row # For fetching columns by name
        cursor = db.cursor()
        keyboard_events = []
