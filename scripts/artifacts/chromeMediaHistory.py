...
class ChromeMediaHistoryPlugin(ArtefactPlugin):
    ...
    def __init__(self):
        ...
        self.name = 'Chrome Media History'
        self.description = ''
    ...
    def _processor(self) -> bool:
        ...
        for file_found in self.files_found:
            ...
            if usageentries > 0:
                ...
                report_path = os.path.join(self.report_folder, f'{browser_name} Media History - Sessions.temphtml')
            ...
            if usageentries > 0:
                ...
                #check for existing and get next name for report file, so report from another file does not get overwritten
                report_path = os.path.join(self.report_folder, f'{browser_name} Media History - Playbacks.temphtml')
                report_path = get_next_unused_name(report_path)[:-9] # remove .temphtml
                report.start_artifact_report(self.report_folder, os.path.basename(report_path))
                report.add_script()
            ...
            if usageentries > 0:
                ...
                #check for existing and get next name for report file, so report from another file does not get overwritten
                report_path = os.path.join(self.report_folder, f'{browser_name} Media History - Origins.temphtml')
                report_path = get_next_unused_name(report_path)[:-9] # remove .temphtml
                report.start_artifact_report(self.report_folder, os.path.basename(report_path))
                report.add_script()
